...
系统诊断插件不随源码仓库提供。GitHub Actions 会从官方插件 Releases 仓库获取最新正式版本，将其作为 `PluginSeed` 注入发布目录，首次启动时登记到用户数据目录的 `Plugins/log-analyzer`。
...
2. 首页会显示检查、等待和分析状态；分析成功后进入分析中心内嵌的报告查看器。
