...
本地联调内置或已安装应用时，从同级 `Workbench-Apps` 仓库执行 `npm run dev:workbench -- <appId>`。该命令只启动未打包的 Workbench 并显式加载目标应用本地 `dist`；正式安装包不识别开发覆盖配置，仍只消费已签名应用包。

## 用户数据

宿主数据库保存桌面图标布局，以及原生应用窗口的普通边界和最大化状态。应用的诊断包、任务、报告和规则数据仍由各自应用 backend 保存到应用数据目录。渲染进程只能通过 preload 暴露的 `window.workbench` 接口访问宿主能力。
