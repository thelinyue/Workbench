...
Report Open Service ──> Windows 默认浏览器 ──> local Report/index.html
...
| Report（默认浏览器） | 查询 reports/case | 读取报告偏好 | 读取 `Report/index.html` | Windows Shell 浏览器启动 | 分析中心历史记录、最后打开时间 |
...
输出：PluginManifest、Issues、外部 EXE 进程结果、`Report/index.html`、插件日志。
风险点：
...
## 12. Report Service 与默认浏览器打开模块

名称：报告查询与安全打开服务
路径：

- `src/HephaestusWorkbench.Services/ReportService.cs`
- `src/HephaestusWorkbench.Services/ReportOpenService.cs`
- `src/HephaestusWorkbench.Core/Services/ReportOpenContracts.cs`

职责：为分析中心查询报告状态，校验 `Report/index.html` 的案例路径边界，并通过 Windows 默认浏览器打开报告。
入口文件：`ReportService.ListAsync`、`IReportOpenService.OpenAsync`。
依赖：Core、Data repositories、CaseAnalysisService、Windows Shell。
输入：`ReportQuery`、`ReportOpenRequest`。
输出：`ReportSummary` 列表、`ReportOpenResult` 和 `reports.last_opened_at`。
风险点：

- 报告目录、入口文件或解压目录出现重解析点时必须拒绝打开。
- 浏览器启动失败不能更新最后打开时间。
- 报告只能在电脑默认浏览器中打开，不能重新引入 WPF/WebView2 报告标签。
