...
脚本在 `installer/dist` 生成单个标准离线安装包 `HephaestusWorkbench_Setup.exe`、`log-analyzer-1.50-win-x64.zip`、市场目录和校验文件。Setup 内含 self-contained 主程序，不依赖联网下载应用载荷。没有插件二进制时，普通源码构建仍然可执行，但正式打包会立即返回中文错误。
...
- Releases 仓库 Release `v1.1.1`：单个离线 Setup、`SHA256SUMS.txt` 和发行说明。
