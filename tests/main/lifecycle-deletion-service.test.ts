...
describe('诊断包完整生命周期删除', () => {
  it('永久删除原始包、解压目录、报告、案例索引和关联任务', async () => {
    ...
    repository.upsertTask({ id: diagnosticPackage.taskIds[0], packageId: diagnosticPackage.id, status: 'succeeded', createdAt: new Date().toISOString(), progress: 100, message: '报告已生成' });
    ...
    expect(preview).toMatchObject({ packageCount: 1, taskCount: 1, sourcePaths: [sourcePath], extractPaths: [extractPath], reportPaths: [reportPath] });
