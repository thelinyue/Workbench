...
describe('应用中心服务', () => {
  it('把目录、已安装版本和兼容性合并为应用状态', async () => {
    ...
    repository.upsert({ id: 'analysis-center', name: '分析中心', description: '旧描述', publisherId: 'thelinyue', installedVersion: '1.0.0', activeVersion: '1.0.0', state: 'installed' });
...
class FakeInstaller {
  ...
  public async installRelease(app: AppCatalogDocumentV1['apps'][number], release: AppCatalogRelease, _payload: Uint8Array): Promise<AppInstallRecord> {
    ...
    return { id: app.id, name: app.name, description: app.description, publisherId: app.publisherId, installedVersion: release.version, activeVersion: release.version, state: 'installed' };
  }
}
