...
public sealed class ReportsWorkspaceViewModelTests
{
    ...
    public async Task OpenReport_DeduplicatesAndClosesOldestAtConfiguredLimit()
    {
        ...
        try
        {
            ...
            var analysis = new CaseAnalysisService(paths, cases, tasks, reports, catalog, new LegacyLogAnalyzerRunner(logger), new StandardExePluginRunner(logger), taskCenter, logger, new SqliteAnalysisLifecycleRepository(factory));
            ...
            using var workspace = new ReportsWorkspaceViewModel(new ReportService(reports, analysis), settings, openedExtractPaths.Add, logger, _ => true);

            var first = Summary(0);
            await workspace.OpenReportAsync(first);
            await workspace.OpenReportAsync(first);
            Assert.Single(workspace.OpenTabs);
            Assert.False(workspace.IsAnalysisListVisible);
            workspace.ShowAnalysisListCommand.Execute(null);
            Assert.True(workspace.IsAnalysisListVisible);
