...
public sealed class ReportsWorkspaceViewModelTests
{
    ...
    public async Task OpenReport_DeduplicatesAndClosesOldestAtConfiguredLimit()
    {
        ...
        try
        {
            ...
            using var workspace = new ReportsWorkspaceViewModel(new ReportService(reports, sessions, analysis), settings, _ => { }, logger, _ => true, _ => true);
    ...
    private static ReportSummary Summary(int index) => new()
    {
        ...
        Path = Path.GetTempPath(), PluginId = "plugin", PluginName = "插件", CreateTime = DateTime.Now.AddMinutes(index), IsAvailable = true
    };
}
