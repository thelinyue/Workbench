...
public sealed class SettingsInformationArchitectureTests
{
    ...
    public void SettingsPage_OrganizesWorkspaceMonitoringSshAndStoragePreferences()
    {
        ...
        Assert.Contains(bindings, value => value.Contains("AllowPrereleaseExtensions", StringComparison.Ordinal));
        Assert.Contains(document.Descendants(), element =>
            (string?)element.Attribute("AutomationProperties.Name") == "启动时自动检查扩展更新");
        Assert.Contains(document.Descendants(), element =>
            (string?)element.Attribute("AutomationProperties.Name") == "允许预发布扩展");
        Assert.DoesNotContain(bindings, value => value.Contains("UpdateChannel", StringComparison.Ordinal));
